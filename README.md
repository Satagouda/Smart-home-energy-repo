...
2) Users can set a custom energy budget (kWh).
3) The system calculates total usage and compares it against the budget.
4) Alerts are shown:
   ✅ Under budget → Success message.
   ⚠️ Over budget → Warning message.
5) A line chart plots the usage trend across devices.
6) A clean UI displays individual device consumption.
